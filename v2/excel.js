/* ===== V2 Excel Export with Styled Borders (xlsx-js-style) ===== */

function exportExcel(){
  const wb = XLSX.utils.book_new();
  const aux = Number(document.getElementById('auxMaterial').value) || 0;
  let sellTotal=0, costTotal=0;
  parts.forEach(p => {const s=floor100(p.sellInput); sellTotal+=s*p.qty; costTotal+=(p.source==='self'?Math.round(s*0.62):p.costPrice)*p.qty;});
  let laborSum=0; labors.forEach(l => laborSum+=l.amount);
  let discSum=0; discounts.forEach(d => discSum+=d.amount);
  ...
  // dFmt = YYYY-MM-DD
  const dFmt = dateVal.replace(/(\d{4})-(\d{2})-(\d{2})/, '$1-$2-$3');
  const installDateVal = document.getElementById('installDate').value;
  const iDFmt = installDateVal ? installDateVal.replace(/(\d{4})-(\d{2})-(\d{2})/, '$1-$2-$3') : '';
  ...
  function setCell(r, c, val, align = 'left', customFont = fontNormal, border = null) {
  ...
  setCell(8, 0, '納入場所', 'center'); setCell(8, 5, document.getElementById('delivPlace').value);
  setCell(10, 0, '納期', 'center'); setCell(10, 5, document.getElementById('delivDate').value);
  setCell(11, 0, '御支払条件', 'center'); setCell(11, 5, document.getElementById('payTerms').value);
  setCell(12, 0, '見積有効期限', 'center'); setCell(12, 5, document.getElementById('validity').value);

  setCell(8, 15, '様', 'center');
  setCell(8, 19, '担当部署', 'center'); setCell(8, 25, document.getElementById('compDept').value);
  setCell(10, 19, 'TEL', 'center'); setCell(10, 20, document.getElementById('compTel').value);
  setCell(10, 26, 'FAX ', 'center'); setCell(10, 27, document.getElementById('compFax').value);
  setCell(11, 21, '担当者', 'center'); setCell(11, 24, staff);
  setCell(13, 26, '上長', 'center'); setCell(13, 29, '担当者', 'center');
  
  // 2. Equipment Information
  setCell(14, 0, '製品名', 'center'); setCell(14, 4, document.getElementById('prodName').value);
  setCell(14, 12, '機種', 'center'); setCell(14, 15, document.getElementById('modelNo').value);
  setCell(15, 0, '設置日', 'center'); setCell(15, 4, iDFmt);
  setCell(15, 12, '機番', 'center'); setCell(15, 15, document.getElementById('serialNo').value);
  
  // 3. Call Content
  setCell(16, 0, '【コール内容】'); setCell(16, 5, document.getElementById('callContent').value);
  setCell(17, 0, '【故障原因・処置】'); setCell(17, 5, document.getElementById('causeAction').value);
  
  setCell(20, 0, '故障箇所の部品のみの修繕内容です。', 'left', {sz: 9, name: 'ＭＳ Ｐゴシック', bold:true});
  
  // 4. Parts Table
  setCell(21, 0, 'No', 'center'); setRangeBorder(21,0,21,0);  
  setCell(21, 1, '部品名', 'center'); setRangeBorder(21,1,21,12);    
  setCell(21, 13, '部品コード', 'center'); setRangeBorder(21,13,21,18);      
  setCell(21, 19, '数量', 'center'); setRangeBorder(21,19,21,21);      
  setCell(21, 22, '単価', 'center'); setRangeBorder(21,22,21,26);      
  setCell(21, 27, '金額', 'center'); setRangeBorder(21,27,21,31); 
  ...
  for(let i=0; i<parts.length && i<15; i++) {
     ...
     setCell(rIdx+i, 22, pt.name?(sell===0?0:sell):'', 'right');
     setCell(rIdx+i, 27, pt.name?(sell*pt.qty===0?0:sell*pt.qty):'', 'right');
  }

  // 5. Labor & Totals
  setCell(37, 0, '補修・補助材料費', 'center'); setRangeBorder(37,0,37,19);
  setCell(37, 20, aux > 0 ? '一式' : '', 'center'); setRangeBorder(37,20,37,21);
  setCell(37, 27, aux||0, 'right'); setRangeBorder(37,22,37,31);

  setCell(38, 0, '使用部品費合計', 'center'); setRangeBorder(38,0,38,19);
  setCell(38, 22, sellTotal+aux, 'right'); setRangeBorder(38,22,38,31);

  setCell(39, 0, '作業名　及び　値引き', 'center'); setRangeBorder(39,0,49,1);
  
  let lIdx = 39;
  labors.forEach(l => {
     if (lIdx <= 49) {
         setCell(lIdx, 2, l.name, 'left'); setRangeBorder(lIdx,2,lIdx,18);
         setCell(lIdx, 19, l.amount>0 ? '一式' : '', 'center'); setRangeBorder(lIdx,19,lIdx,21);
         setCell(lIdx, 22, l.amount||0, 'right'); setRangeBorder(lIdx,22,lIdx,31);
         lIdx++;
     }
  });
  discounts.forEach(d => {
     if (lIdx <= 49) {
         setCell(lIdx, 2, d.name, 'left'); setRangeBorder(lIdx,2,lIdx,18);
         setCell(lIdx, 19, '', 'center'); setRangeBorder(lIdx,19,lIdx,21);
         setCell(lIdx, 22, d.amount ? -d.amount : 0, 'right'); setRangeBorder(lIdx,22,lIdx,31);
         lIdx++;
     }
  });
  for(; lIdx<=49; lIdx++) {
  ...
  setCell(50, 19, '小計', 'center'); setRangeBorder(50,19,50,23);
  setCell(50, 24, subtotal, 'right'); setRangeBorder(50,24,50,31);
  setCell(51, 19, '消費税', 'center'); setRangeBorder(51,19,51,23);
  setCell(51, 24, tax, 'right'); setRangeBorder(51,24,51,31);
  setCell(52, 19, '合計', 'center'); setRangeBorder(52,19,53,23);
  setCell(52, 24, total, 'right'); setRangeBorder(52,24,53,31);

  // 6. Reply section
  setCell(56, 0, '御返答欄', 'center'); setRangeBorder(56,0,61,1); 
  setCell(56, 2, '上記御見積書の御返答を御願い致します。', 'left');

  setCell(58, 3, '１. 修理する');
  setCell(58, 16, '御返答日'); setCell(58, 22, '年', 'center'); setCell(58, 25, '月', 'center'); setCell(58, 28, '日', 'center');
  
  setCell(60, 3, '２. 修理しない');
  setCell(60, 16, '発注者'); setCell(60, 25, '様'); setCell(60, 27, '印', 'center');

  // Borders for reply block exterior
  ...
  if(remarksText) {
      ...
      if(remarks[0]) { setCell(62, 0, '＊', 'right', fontSmall); setCell(62, 1, remarks[0], 'left', fontSmall); }
      if(remarks[1]) { setCell(63, 0, '＊', 'right', fontSmall); setCell(63, 1, remarks[1], 'left', fontSmall); }
  }
...
function resetAll(){
  ...
  parts=[];labors=[];discounts=[];
  renderParts();renderLabor();renderDiscount();recalc();
  initSampleData();
...
initTheme();
if(!autoLoad()){initSampleData();}
renderTemplates();
updateTabBadges();
document.addEventListener('input',()=>{autoSave();updateTabBadges();});
